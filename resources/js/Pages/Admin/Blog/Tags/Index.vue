<template>
    <AdminLayout>
        <template #head>
            <title>{{ $t('dashboard') }} | {{ $t('tags') }}</title>
        </template>

        <div class="mb-4">
            <h3 class="text-muted">{{ $t('tags') }}</h3>
        </div>
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between" style="border: none;">
                <span>{{ $t('tags') }}</span>
                <button class="btn btn-sm btn-primary" @click="create">{{ $t('create_new_tag') }}</button>
            </div>
            <div class="card-body">
                <DataTable :data='tags' :url='route("admin.tags.index")' :columns="columns">
                    <template #body="{ items }">
